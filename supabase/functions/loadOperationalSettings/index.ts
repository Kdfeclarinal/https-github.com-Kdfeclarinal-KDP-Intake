...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const [{ data: users, error: usersError }, { data: grants, error: grantsError }, { data: defaultRow, error: defaultError }, { data: basecamp, error: basecampError }] = await Promise.all([
    ...
    if (usersError || grantsError || defaultError || basecampError) throw new Error('Operational settings load failed.');
    ...
    const body = sanitizeOperationalSettings({
      ...
      defaultReviewerId: defaultRow?.reviewer_user_id, employees,
      integrations: {
