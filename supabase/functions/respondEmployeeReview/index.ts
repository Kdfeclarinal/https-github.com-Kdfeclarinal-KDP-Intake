...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (!bookId || !token || !commentId || !text.trim()) return json({ ok: false, error: 'Employee reply is invalid.' }, 400)
    const supabase = createClient(Deno.env.get('SUPABASE_URL') || '', Deno.env.get('SUPABASE_SERVICE_ROLE_KEY') || '', { auth: { persistSession: false } })
    const { data, error } = await supabase.rpc('add_employee_review_reply', { p_book_id: bookId, p_token_hash: await sha256(token), p_comment_id: commentId, p_body: text })
