...
(async () => {
  ...
  await page.route('**/functions/v1/loadEmployeePage', (route) => {
    ...
    const body = submissions.length
      ? { ...payload(request.step_name), book: { ...payload(request.step_name).book, overall_status: 'AWAITING_REVIEW' }, employee_mode: 'submitted', can_edit: false }
      : payload(request.step_name);
  ...
  await page.getByRole('heading', { name: 'Submitted for Review' }).waitFor();
  ...
  check(await page.locator('.kdp-app--pricing').count() === 0, 'successful submission immediately leaves the editable Pricing UI');
  check(await page.getByRole('button', { name: /Save|Submit|Upload/i }).count() === 0, 'submitted state exposes no mutation action');
